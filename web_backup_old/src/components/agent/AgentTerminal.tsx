...
export function AgentTerminal({ onCodeGenerated }: AgentTerminalProps) {
  ...
  const handleTemplateSelect = (template: ProjectTemplate) => {
    setSpecification(template.prompt);
    setShowTemplates(false);
    setMessages((prev) => [
      ...prev,
      {
        type: 'system',
        content: `✨ Выбран шаблон: ${template.name}`,
        timestamp: new Date(),
      },
    ]);
  };
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <Button
              ...
              className={`text-xs ${showTemplates ? 'bg-indigo-500/20 text-indigo-400' : 'text-zinc-400 hover:text-white'}`}
            >
              {showTemplates ? '💬 Чат' : '📦 Шаблоны'}
            </Button>
          </div>
        </div>
      </CardHeader>
